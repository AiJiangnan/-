...
\begin{enumerate}
 ...
 hf = figure('Name','MyFigure',...					% 设置窗口名称
	...
	\qquad 在设计菜单的时候，必须要先有一个图形框架窗口，我们在前面用figure函数创建好的界面基础上设计菜单栏。设计的方法\footnote{参考文献[2]第9章Matlab GUI的组成与结构第183页}如下：
	\begin{lstlisting}
 hm = uimenu(parent,'PropertyName',propertyvalue,...);\end{lstlisting}
	...
	\begin{lstlisting}
 hm1 = uimenu(hf,'label','a_menu');
 hm2 = uimenu(hf,'label','b_menu');
 hm3 = uimenu(hf,'label','c_menu');
 subhm1 = uimenu(hm2,'label','b_menu_1');
 subhm2 = uimenu(hm2,'label','b_menu_2');
 subhm3 = uimenu(hm2,'label','b_menu_3');\end{lstlisting}
\end{enumerate}
